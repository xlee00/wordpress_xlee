...
function siteorigin_premium_admin_menu() {
	...
	$premium_name = apply_filters('siteorigin_premium_theme_name', ucfirst( get_option( 'template' ) ) . ' ' . __( 'Premium', 'puro' ) );
	...
	switch ( $action ) {
		...
		case 'enter-order' :
			...
			<div class="wrap" id="theme-upgrade">
				<h2><?php printf(__('Your Order Number is [%s]', 'puro'), get_option( $option_name )) ?></h2>
...
function siteorigin_premium_default_content($content){
	...
	$premium_name =  apply_filters('siteorigin_premium_theme_name', ucfirst($theme) . ' ' . __( 'Premium', 'puro' ) );

	$content['rewards'][] = array(
		'amount' => 10,
		'title' => sprintf(__('A Copy of %s Premium', 'puro'), ucfirst($theme)),
		'text' => sprintf(__('You get a copy of %s, delivered instantly to your PayPal email address. This includes the same basic support we offer users of our free themes.', 'puro'), $premium_name ),
	);
	...
	$content['rewards'][] = array(
		...
		'text' => sprintf(__("We'll go the extra mile and help you with minor CSS customizations, plugin conflicts and anything else that falls outside standard %s support.", 'puro'), $premium_name ),
	);
